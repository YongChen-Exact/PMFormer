...
def get_case_identifier(case):
    # print(case)
    case_identifier = case[0].split("/")[-1].split(".nii.gz")[0][-12:-5]
...
def get_case_identifier_from_npz(case):
    case  = case.replace("\\","/")
    case_identifier = case.split("/")[-1][-16:-4]
...
def crop_to_nonzero(data, seg=None, nonzero_label=0):# 这儿是坑，再PMFormer下面可以为-1，但是再其他地方不行
    """
...
def get_patient_identifiers_from_cropped_files(folder):
    ...
    return [i.split("\\")[-1][-15:-4] for i in subfiles(folder, join=True, suffix=".npz")]
