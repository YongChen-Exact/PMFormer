...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, tag=None):
        ...
        self.dim = dim
        if tag == 0:
            self.reduction = nn.Conv3d(dim * 2, dim * 2, kernel_size=[1, 3, 3], stride=[1, 2, 2], padding=[0, 1, 1])
        elif tag == 1:
            self.reduction = nn.Conv3d(dim * 2, dim * 2, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[1, 1, 1])
        else:
            self.reduction = nn.Conv3d(dim * 2, dim * 2, kernel_size=[3, 3, 3], stride=[2, 2, 2], padding=[0, 1, 1])
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.scale_img2 = nn.AvgPool3d((1, 2, 2), (1, 2, 2))
...
class PMFormer(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[64, 128, 128],
        ...
        self.model_down = Encoder(pretrain_img_size=crop_size, window_size=window_size, embed_dim=embed_dim,
                                  ...
                                  down_stride=down_stride, conv_op=self.conv_op, norm_op=norm_op,
        ...
        self.decoder = Decoder(pretrain_img_size=crop_size, embed_dim=embed_dim, window_size=window_size[::-1][1:],
                               num_heads=num_heads[::-1][1:], depths=depths[::-1][1:],
